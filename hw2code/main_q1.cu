...
const char kMobyDick[] = "common/mobydick.txt";
...
void checkResults(std::vector<unsigned char>& text_host
    ...
    for(int i = 0; i < num_entries; i++) {
        if(text_host[i] != text_from_gpu[i]) {
            ++error;
            std::cerr << "Mismatch at pos: " << i << "\nExpected " << (int)text_host[i] <<
                      " and got " << (int)text_from_gpu[i] << std::endl;
...
int main(int argc, char** argv) {
    ...
    if(argc != 2) {
        std::cerr << "Must supply the number of times to double the input file!" <<
                  std::endl;
        return 1;
    }
    ...
    unsigned char shift_amount = (rand() % 25) +
                                 1; //we don't want the shift to be 0!
    ...
    // copy input to GPU
    {
        ...
        std::cout << "Host -> Device transfer bandwidth " << num_bytes /
                  (elapsed_time_h2d / 1000.) / 1E9 << std::endl << std::endl;
    }
    ...
    // generate reference output
    {
        ...
        std::cout << "Host (reference) solution bandwidth GB/sec: " << 2 * num_bytes /
                  (elapsed_time_host / 1000.) / 1E9 << std::endl << std::endl;
    }
    ...
    std::cout << std::setw(70) << std::setfill('-') << " " << std::endl <<
              std::setfill(' ');
    std::cout << std::setw(15) << " " << std::setw(15) << "char" << std::setw(
                  15) << "uint" << std::setw(15) << "uint2" << std::endl;
    ...
    for(uint i = 0; i < sizes_to_test.size(); ++i) {
        // generate GPU char output
        double elapsed_time_char = doGPUShiftChar(device_input_array,
                                   device_output_array, shift_amount, sizes_to_test[i], block_size);
        ...
        std::cout << std::setw(15) << sizes_to_test[i] / 1E6 << " " <<
                  ...
                  std::setw(15) << 2 * sizes_to_test[i] / (elapsed_time_uint2 / 1000.) / 1E9 <<
                  std::endl;
    }
