...
void checkErrors(const std::vector<float>& h_gpu_node_values_output,
    ...
    for(uint i = 0 ; i < h_gpu_node_values_output.size() ; i++) {
        ...
        if(!AlmostEqualUlps(n,c,kMaxUlps)) {
            num_errors++;
...
int main(void) {
    //initialize CUDA and warmup kernel to avoid including these costs in the timings
    cudaFree(0);
    ...
    std::cout << std::setw(60) << "Device Bandwidth GB/sec" << std::endl <<
              std::endl;
    std::cout << std::setw(15) << "edges \\ nodes";

    for(std::vector<uint>::iterator nodes = num_nodes.begin();
            nodes != num_nodes.end(); ++nodes) {
        std::cout << std::setw(15) << *nodes;
    }

    std::cout << std::endl << std::endl;

    for(std::vector<uint>::iterator edges = avg_edges.begin();
            edges != avg_edges.end(); ++edges) {
        ...
        for(std::vector<uint>::iterator nodes = num_nodes.begin();
            ...
            host_graph_iterate(h_graph_indices.data(),
                               h_graph_edges.data(),
                               h_node_values_input.data(),
            ...
            size_t uint_bytes = 2*(*nodes)*iterations*(1 + (*edges))*sizeof(uint);
            size_t float_bytes = (*nodes)*iterations*(1 + 2*(*edges))*sizeof(float);
            size_t totalBytes = uint_bytes + float_bytes;
            std::cout << std::setw(15) << std::fixed << std::setprecision(
                          2) << totalBytes / (gpu_time / 1000.) / 1E9 << std::flush;
        }
